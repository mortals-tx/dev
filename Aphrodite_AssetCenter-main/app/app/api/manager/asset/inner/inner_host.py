...
from app.models.asset import InnerHost, Zone
from app.libs.success_types import Success


innerhost = Redprint('innerhost')


@innerhost.route('', methods=['GET'])
@auth.login_required
def get_list():
...
@innerhost.route('/service', methods=['GET'])
@auth.login_required
def get_service():
    uid = request.args.get('uid', '', type=str)
    h = InnerHost.get_item_by_uid(uid=uid)
    data = {
...
@innerhost.route('/info', methods=['POST'])
@auth.login_required
def update_info():
...
def save_info():
    ...
    with db.auto_commit():
        InnerHost.save(ip=ip, origin=origin, zone_uid=zone_uid)
    return Success()


@innerhost.route('/activate', methods=['POST'])
@auth.login_required
def activate():
    data = request.get_json(silent=True)
    uid = data.get('uid')
    h = InnerHost.get_item_by_uid(uid, status=0)
    with db.auto_commit():
        h.activate()
    return Success()


@innerhost.route('/remove', methods=['POST'])
@auth.login_required
def remove():
    data = request.get_json(silent=True)
    uid = data.get('uid')
    h = InnerHost.get_item_by_uid(uid)
    with db.auto_commit():
        h.remove()
    return Success()
