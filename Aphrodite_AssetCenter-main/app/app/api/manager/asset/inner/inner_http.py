...
from app.models.asset import InnerHttp, Zone
from app.libs.success_types import Success
import json

innerhttp = Redprint('innerhttp')


@innerhttp.route('', methods=['GET'])
@auth.login_required
def get_list():
...
# 管理台-修改
@innerhttp.route('/info', methods=['POST'])
@auth.login_required
def update_info():
    ...
    info = data.get('info')
    h = InnerHttp.get_item_by_uid(uid=uid)
    with db.auto_commit():
...
def save_info():
    ...
    with db.auto_commit():
        InnerHttp.save(website=website, http_status=http_status, title=title, size=size, cms=cms, product=product, zone_uid=zone_uid, info=info)
    return Success()

@innerhttp.route('/activate', methods=['POST'])
@auth.login_required
def activate():
    data = request.get_json(silent=True)
    uid = data.get('uid')
    h = InnerHttp.get_item_by_uid(uid, status=0)
    with db.auto_commit():
        h.activate()
    return Success()


@innerhttp.route('/remove', methods=['POST'])
@auth.login_required
def remove():
    data = request.get_json(silent=True)
    uid = data.get('uid')
    h = InnerHttp.get_item_by_uid(uid)
    with db.auto_commit():
        h.remove()
    return Success()
