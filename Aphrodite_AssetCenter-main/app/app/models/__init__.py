...
class BaseModel(db.Model, MixinJSONSerializer):
    __abstract__ = True
    # 默认字段
    uid = Column(UUID, primary_key=True, nullable=False) # UID 标识
    utc_created = Column(DATETIME, nullable=False) # 创建时间
    utc_modified = Column(DATETIME, nullable=False) # 修改时间
    status = Column(TINYINT(unsigned=True), default=1, nullable=False) # 状态
    ...
    def update(self, **kwargs):
        ...
        self.utc_modified = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        db.session.add(self)
