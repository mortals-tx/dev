    ...
    >
      ...
      <a-form :form="infoForm">
        ...
        <a-form-item
          ...
          label="name"
        >
          <a-input placeholder="域名" v-model="infoParams.name" id="domain_name"/>
        </a-form-item>
        <a-form-item
          :labelCol="{xs: { span: 24 },sm: { span: 5 }}"
          :wrapperCol="{xs: { span: 24 }, sm: { span: 16 }}"
          label="解析IP"
        >
          <a-input placeholder="解析IP" v-model="infoParams.resolve_ip" id="domain_resolve_ip"/>
        </a-form-item>
        <a-form-item
          :labelCol="{xs: { span: 24 },sm: { span: 5 }}"
          :wrapperCol="{xs: { span: 24 }, sm: { span: 16 }}"
          label="origin"
        >
          <a-input placeholder="来源" v-model="infoParams.origin" id="domain_origin"/>
        </a-form-item>
...
import {
  ...
  saveDomainInfo,
  updateDomainInfo
} from '@/api/manager/asset/outside/domain'
...
export default {
  ...
  data () {
    return {
      ...
      loadData: parameter => {
        ...
        return getDomainList(params).then(response => {
  ...
  methods: {
    ...
    handleActivateDomain (record) {
      ...
      activateDomain({ 'uid': record.uid }).then(response => {
    ...
    handleRemoveDomain (record) {
      ...
      removeDomain({ 'uid': record.uid }).then(response => {
        this.$refs.table.refresh(true)
      })
    ...
    handleOk () {
      if (this.state.isSaveInfoEditor) {
        saveDomainInfo(this.infoParams).then(response => {
          this.$refs.table.refresh()
        })
      } else {
        updateDomainInfo(this.infoParams).then(response => {
          this.$refs.table.refresh()
        })
      }
