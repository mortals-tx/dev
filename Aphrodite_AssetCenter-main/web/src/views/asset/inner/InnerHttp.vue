    ...
    <s-table
      ...
      <span slot="action" slot-scope="record">
        <a
          ...
          :key="record.uid"
          @click="handleRemoveInnerHttp(record)"
        ...
        <a
          v-show="record.status !== 1"
          v-action:activate
          @click="handleActivateInnerHttp(record)"
...
import { activateInnerHttp, getInnerHttpList, removeInnerHttp, saveInnerHttpInfo, updateInnerHttpInfo } from '@/api/manager/asset/inner/innerhttp'
...
export default {
  ...
  data () {
    return {
      ...
      loadData: parameter => {
        ...
        return getInnerHttpList(params).then(response => {
  ...
  methods: {
    ...
    handleActivateInnerHttp (record) {
      this.state.activateBtn = true
      activateInnerHttp({ 'uid': record.uid }).then(response => {
        this.$refs.table.refresh(true)
      })
      this.state.activateBtn = false
    },
    handleRemoveInnerHttp (record) {
      this.state.removeBtn = true
      removeInnerHttp({ 'uid': record.uid }).then(response => {
        this.$refs.table.refresh(true)
      })
    ...
    handleOk () {
      if (this.state.isSaveInfoEditor) {
        saveInnerHttpInfo(this.infoParams).then(response => {
          this.$message({
            message: 'success',
            type: 'success'
          })
        ...
        updateInnerHttpInfo(this.infoParams).then(response => {
          this.$message({
            message: 'success',
            type: 'success'
          })
