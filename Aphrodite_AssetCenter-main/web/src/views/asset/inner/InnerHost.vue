  ...
  <a-card :bordered="false" v-action:get_list>
        ...
        <a-row :gutter="48">
          <a-col :md="6" :sm="24">
            <a-form-item label="IP">
              <a-input v-model="filterParams.ip" placeholder="请输入" />
            </a-form-item>
          </a-col>
          <a-col :md="6" :sm="24">
            <a-form-item label="来源">
              <a-input v-model="filterParams.origin" placeholder="请输入" />
            </a-form-item>
          </a-col>
          <a-col :md="6" :sm="24">
    ...
    <div class="table-operator">
      <a-button
        type="dashed"
        @click="tableOption"
      >{{ (state.optionAlertShow && '关闭') || '开启' }} alert</a-button
      >
    </div>
    ...
    >
      <div slot="expandedRowRender" slot-scope="record" style="margin: 0">
        ...
        <a-row v-if="record" :gutter="24" :style="{ marginBottom: '12px' }">
          <a-list bordered :dataSource="record">
            <a-collapse v-action:get_service @change="changeActiveKey">
              <a-collapse-panel header="services" :key="record.uid">
                <s-table
                  ...
                  :rowKey="service => service.uid"
                  :pageSize.sync="servicePageSize"
                >
                  <pre
      ...
      <span slot="action" slot-scope="record">
        <a
          ...
          :key="record.uid"
          @click="handleRemoveInnerHost(record)"
        ...
        <a
          v-action:activate
          v-show="record.status !== 1"
          @click="handleActivateInnerHost(record)"
...
import {
  ...
  saveInnerHostInfo,
  updateInnerHostInfo
} from '@/api/manager/asset/inner/innerhost'
...
export default {
  ...
  data () {
    return {
      ...
      loadData: parameter => {
        ...
        return getInnerHostList(params).then(response => {
      ...
      loadServiceData: parameter => {
        return getInnerHostService({ uid: this.currentInnerHostUid }).then(
          response => {
            const msg = response.msg
  ...
  methods: {
    ...
    handleActivateInnerHost (record) {
      this.state.activateBtn = true
      activateInnerHost({ uid: record.uid }).then(response => {
        this.$refs['table'].refresh(true)
      })
      this.state.activateBtn = false
    },
    handleRemoveInnerHost (record) {
      this.state.removeBtn = true
      removeInnerHost({ uid: record.uid }).then(response => {
        this.$refs['table'].refresh(true)
      })
    ...
    handleOk () {
      if (this.state.isSaveInfoEditor) {
        saveInnerHostInfo(this.infoParams).then(response => {
          this.$refs.table.refresh()
        })
      } else {
        updateInnerHostInfo(this.infoParams).then(response => {
          this.$refs.table.refresh()
        })
      }
    ...
    changeActiveKey (key) {
      if (key.length !== 0) {
        this.currentInnerHostUid = key[0]
