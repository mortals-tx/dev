    ...
    <s-table
      ...
      <span slot="action" slot-scope="record">
        <a
          ...
          :key="record.uid"
          @click="handleRemoveIpv6Host(record)"
        ...
        <a
          v-action:activate
          v-show="record.status !== 1"
          @click="handleActivateIpv6Host(record)"
    ...
    >
      ...
      <a-form :form="infoForm">
        ...
        <a-form-item
          ...
          label="ip"
        >
          <a-input placeholder="IPV6" v-model="infoParams.ipv6" id="ipv6"/>
        </a-form-item>
        ...
        <a-form-item
          ...
          label="origin"
        >
          <a-input placeholder="来源" v-model="infoParams.origin" id="origin"/>
        </a-form-item>
...
import {
  ...
  saveIpv6HostInfo,
  updateIpv6HostInfo
} from '@/api/manager/asset/ipv6/ipv6host'
...
export default {
  ...
  data () {
    return {
      ...
      loadData: parameter => {
        ...
        return getIpv6HostList(params).then(response => {
      ...
      loadServiceData: parameter => {
        return getIpv6HostService({ 'uid': this.currentIpv6HostUid }).then(response => {
          const msg = response.msg
  ...
  methods: {
    ...
    handleActivateIpv6Host (record) {
      this.state.activateBtn = true
      activateIpv6Host({ 'uid': record.uid }).then(response => {
        this.$refs['table'].refresh(true)
      })
      this.state.activateBtn = false
    },
    handleRemoveIpv6Host (record) {
      this.state.removeBtn = true
      removeIpv6Host({ 'uid': record.uid }).then(response => {
        this.$refs['table'].refresh(true)
      })
    ...
    handleOk () {
      if (this.state.isSaveInfoEditor) {
        saveIpv6HostInfo(this.infoParams).then(response => {
          this.$refs.table.refresh()
        })
      } else {
        updateIpv6HostInfo(this.infoParams).then(response => {
          this.$refs.table.refresh()
        })
      }
